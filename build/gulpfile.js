...
gulp.task('release',
	[
		...
		fs.readFile( '../extensions/components/com_' + extension.name + '/' + extension.name + '.xml', function(err, data) {
...
gulp.task('release:component', ['clean:release', 'release:prepare-component', 'release:prepare-redcore'], function (cb) {
	fs.readFile( '../extensions/components/com_' + extension.name + '/' + extension.name + '.xml', function(err, data) {
		parser.parseString(data, function (err, result) {
			var version = getVersion(result);
...
gulp.task('release:prepare-component', function () {
	return gulp.src([
			'../extensions/components/com_' + extension.name + '/**/*'
		])
		.pipe(gulp.dest('tmp'));
});
...
gulp.task('release:languages', ['clean:release'], function() {
	var langPath = '../extensions/languages';
